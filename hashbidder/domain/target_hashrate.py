...
def compute_needed_hashrate(target: Hashrate, current_24h: Hashrate) -> Hashrate:
    ...
    Assumes a 12-hour horizon: if we run at `needed` for the next 12 hours, the
    rolling 24h average will land at `target` (via 12·current + 12·needed = 24·target).
    Clamped to zero when current_24h >= 2·target. Can't be between 0 and 1.
    """
    target_ph_s = target.to(HashUnit.PH, TimeUnit.SECOND).value
    current_ph_s = current_24h.to(HashUnit.PH, TimeUnit.SECOND).value

    needed_value = Decimal(2) * target_ph_s - current_ph_s
    if needed_value <= 0:
